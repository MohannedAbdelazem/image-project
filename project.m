    ...
    for c = 1:channels
        for i = 1:rows
            for j = 1:cols
                ...
                noise_variance = 0.01; % Adjust based on noise
                output_image(i, j, c) = local_mean + max(0, local_variance - noise_variance) / max(local_variance, noise_variance) * (input_image(i, j, c) - local_mean);
            end
        end
    end
    output_image = uint8(output_image);
...
output_tv = tv_denoising(input_image, 0.1, 50); 
